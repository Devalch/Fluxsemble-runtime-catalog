...
policies = [
    ...
    ("clone3 unavailable fallback", "jump(BPF_JMP_JEQ_K, libc::SYS_clone3 as u32, 0, 1),\n        statement(BPF_RET_K, SECCOMP_RET_ERRNO | libc::ENOSYS as u32)", 1),
...
test_policies = [
    ...
    ("clone3 ENOSYS assertion", 'line == "clone3=38"', 1),
    ("real pthread fallback probe", "pthread_create(&pthread, NULL, pthread_main, NULL)", 1),
    ("successful pthread fallback assertion", 'line == "pthread_create=0"', 1),
...
for label, old, new in [
    ...
    ("clone3 fallback compatibility bypass", "SECCOMP_RET_ERRNO | libc::ENOSYS as u32", "SECCOMP_RET_ERRNO | libc::EPERM as u32 /* clone3 fallback disabled */"),
...
sources = {
    ...
    "local_tests": Path("crates/catalog-publish/tests/local_staging.rs").read_text(encoding="utf-8"),
...
policies = [
    ...
    ("workflow", "existing upload name reconciliation", 'matching_remote_asset(&before.assets, &expected)?.clone()', 1),
    ("workflow", "order-independent admitted asset set", 'let admitted = &expected[..remote.len()];', 1),
    ("workflow", "remote asset unique IDs", '!ids.insert(actual.asset_id.as_str())', 2),
    ("workflow", "remote asset unique names", '!names.insert(actual.name.as_str())', 2),
    ...
    ("local", "object unsupported unnamed fallback", 'open_exclusive_state_file(objects, &source.sha256, 0o600)?', 1),
    ("local", "object fallback exact link policy", 'let expected_links = if unnamed { 0 } else { 1 };', 1),
    ("local", "latest unsupported unnamed fallback", 'open_exclusive_state_file(&state.latest, LATEST_TEMP, 0o600)?', 1),
    ("local", "latest fallback visibility policy", '!secure_file_allow_links(&metadata, u64::from(temporary.visible))', 1),
    ...
    ("workflow", "pre-post exact prior asset set", 'after.iter().find(|asset| asset.name == prior.name) != Some(prior)', 1),
    ...
    ("security", "local staging unsupported unnamed fallback", 'Digest-addressed local publication objects and the fixed latest-reference temporary use the same unsupported-`O_TMPFILE` allowlist', 1),
...
test_policies = [
    ...
    ("local_tests", "local staging fallback evidence", 'unsupported_unnamed_files_use_no_clobber_local_staging_fallback', 1),
    ("local_tests", "latest fallback preservation evidence", 'named_latest_fallback_failure_before_recovery_binding_is_preserved', 1),
    ...
    ("recovery_tests", "name-reordered exact resume evidence", 'name_reordered_remote_assets_resume_without_reupload', 1),
...
semantic_mutations = [
    ...
    ("workflow", "prior asset-set bypass", "after.iter().find(|asset| asset.name == prior.name) != Some(prior)", "false /* after before asset set */"),
    ("workflow", "remote asset name-set bypass", "!names.insert(actual.name.as_str())", "false /* duplicate remote asset name */"),
