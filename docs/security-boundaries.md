...
Each invocation creates a new empty mode-`0700` HOME and clears the entire child environment. The only child variables are `HOME`, retained `/proc/self/fd/N` `GH_CONFIG_DIR`, `LANG=C`, `LC_ALL=C`, and `TZ=UTC`. No token, proxy, SSH-agent, Git, Cargo/Rust, XDG, `PATH`, repository, user home, or SSH directory is inherited. Six of the seven typed requests map to fixed no-shell `gh api` REST argument arrays with fixed methods, routes, headers, and canonical JSON bodies. Upload maps only to `gh release upload TAG PRIVATE_EXACT_FILE --repo OWNER/NAME` without `--clobber`: the request carries both the validated release tag and release ID, but neither can introduce an argument. The broker never invokes authentication/token commands or accepts arbitrary arguments, routes, methods, hosts, headers, or query strings.

One poll-based nonblocking state machine supervises child stdin, stdout, stderr, leader status, descendant-held pipes, and streamed download writes from a deadline established before spawn. JSON stdout is capped at 128 KiB and stderr at 64 KiB; the first overflow byte, timeout, signal, nonzero exit, malformed/duplicate output, projection mismatch, or a pipe retained by descendants kills the complete process group, closes broker pipe ends, reaps the leader, and produces only `github broker failed`. Download stdout is always piped through the broker, incrementally counted and hashed, and is never written past the fixed asset ceiling. A successful download is fsynced, changed from fresh no-clobber mode `0600` to `0400`, rehashed, read back, and named-rebound; failures unlink only the exact retained fresh inode.

For upload, the broker first authenticates the owner-private mode-`0400` source descriptor, then copies its verified bytes into a fresh broker-owned mode-`0700` per-request directory under the exact validated asset filename. That mode-`0400` no-clobber file is fsynced, hashed, read back, and rebound immediately before spawn; no host input pathname or upload descriptor is inherited. Success returns only typed `asset_uploaded` status with exact name, size, and SHA-256. It never fabricates a remote asset ID; the publisher must issue `read_draft` to bind the server-assigned ID. Neither child bytes, credential/config canaries, request content, nor host paths are echoed.
