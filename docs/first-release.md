...
A fresh confined UI journey then verified XTest pointer and keyboard runtime selection, command-palette parity, review cancellation, separate confirmation without submission, focus containment and return, and one keyboard **Check now** action. The sole qualified desktop/daemon process pair produced exactly one durable `catalog_check` receipt and ordered checking/succeeded events at cursors 6–7, with no operation or installation. A bounded daemon-restart interval preserved the accepted catalog, Pi selection, and one command receipt without creating a duplicate. The live stale-data announcement was not observed during transparent restart and is not claimed; the focused reconnect tests cover announcement deduplication, disconnect during download, remount, cursor gaps, stale events, focus preservation, and duplicate-command rejection.

Task 21 is complete under the release owner's approved Step 5 amendment. Six targeted verification groups passed across the affected consumer runtime, accessibility, transport, persistence, archive/Btrfs, producer publication, conformance, boundary, type-check, compile-check, and frontend-build surfaces. The complete consumer and producer gates remain `not_run` and are not represented as passing; they must run before branch integration, release replacement, or a subsequent production-code change. Final closure review accepted Steps 1–4 and the amended Step 5, and the final security re-review reported the complete driver-source confinement finding addressed with no new blocker. The UI-to-receipt relation remains bounded sole-client temporal correlation rather than cryptographic per-action proof. No tag, release, asset, signature, or publication state changed.
